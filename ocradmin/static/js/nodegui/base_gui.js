...
OCRJS.NodeGui.BaseGui = OCRJS.OcrBase.extend({
    ...
    updateElement: function(element, src) {
        ...
        console.log("Updating element in base");
    ...
    addTransformableRect: function(startpos, css, movedfunc) {
        //console.log("Adding transformable rect at", startpos.x0, startpos.y0, startpos.x1, startpos.y1);
        var self = this;                              
        var rect = $("<div></div>")
    ...
    getSourceRect: function(x0, y0, x1, y1) {
        var viewport = this._viewer.activeViewer().viewport,
            size = this._viewer.activeViewer().source.dimensions,
            elem = $(this._viewer.activeViewer().drawer.elmt);

        var topleft = viewport.pointFromPixel(
                new Seadragon.Point(x0 - elem.offset().left,
                    y0 - elem.offset().top));
        var bottomright = viewport.pointFromPixel(
                new Seadragon.Point(x1 - elem.offset().left, 
                    y1 - elem.offset().top));
        console.log("Screen rect", x0, y0, x1, y1);
        return {
            x0: topleft.x * size.x,
            y0: topleft.y * size.x,
            x1: bottomright.x * size.x,
            y1: bottomright.y * size.x,
        };
    },                       
